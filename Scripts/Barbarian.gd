...
func _ready():
	...
	stealth = get_parent().get_parent().get_node("stealth")
...
func _process(delta):
	...
	if state == "woke":
		...
		animframe = $AnimatedSprite.get_frame()
...
func searching():
	...
	$AnimatedSprite.set_animation("Standing")


	if animationtimer <= 9:
		if facing == "left":
			$AnimatedSprite.set_offset(flip)
		else:
			$AnimatedSprite.set_offset(regular)
...
func sleeping():
	...
	$shadow.set_position(shadowpos)
	$shadow.set_texture(barbshadowsleep)
	$AnimatedSprite.flip_h = false
	$AnimatedSprite.set_animation("Sleeping")
	$AnimatedSprite._set_playing(true) 
	$effect.visible = true

func wake_up():
	$AnimatedSprite.set_animation("Awaking")
	$AnimatedSprite._set_playing(true) 
	$effect.visible = false
...
func get_up():
	everwoke = true
	# change animation
	$AnimatedSprite.set_animation("GetUp")
